...
import com.yasinkacmaz.playground.ui.widget.BottomArcShape
import com.yasinkacmaz.playground.ui.widget.SpacedRow
import com.yasinkacmaz.playground.util.toBackdropUrl
...
@Composable
fun StackScope.BackIcon(onClick: () -> Unit) {
    Icon(
        asset = vectorResource(id = R.drawable.ic_arrow_back),
        tint = Color.White,
        modifier = Modifier.align(Alignment.TopStart).offset(x = 16.dp, y = 16.dp).zIndex(4f).size(24.dp)
        ...
        repeat(starCount) { starIndex ->
            ...
            val (tint, resId) = when {
                voteStarCount >= starIndex + 1 -> {
                    Color.Magenta to R.drawable.ic_star
                }
                voteStarCount in starIndex.toDouble()..(starIndex + 1).toDouble() -> {
                    Color.Magenta to R.drawable.ic_star_half
                }
                else -> {
                    Color.DarkGray to R.drawable.ic_star_empty
                }
            }
            Icon(asset = vectorResource(id = resId), tint = tint)
        }
    }
}
